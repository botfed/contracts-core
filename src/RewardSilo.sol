    ...
    ISilo
{
    ...
    /// @notice Amount minted in the last mintRewards() call
    /// @dev Drips linearly from 0 to this amount over DRIP_DURATION_SECONDS
    ...
    function mintRewards(uint256 amount) external onlyOwner whenNotPaused {
        // Preserve any previously dripped but unwithdrawn rewards
        ...
        // Mint rewards to this contract (BotUSD enforces its own limits)
        asset.mintRewards(address(this), amount);
        emit Minted(amount, block.timestamp);
    }

    /**
     * @notice Calculates total BotUSD available for withdrawal
     * @dev Returns sum of:
     *      - accumulated: previously dripped but unwithdrawn rewards
     *      - newlyDripped: linear portion of lastMintAmount based on time elapsed
     * 
     * Drip formula: (lastMintAmount * elapsed) / DRIP_DURATION_SECONDS
     * - At t=0: newlyDripped = 0
     * - At t=DRIP_DURATION_SECONDS: newlyDripped = lastMintAmount
     * - Linear interpolation in between
     * 
     * @return Total available balance for withdrawal
     */
    function maxWithdrawable() public view returns (uint256) {
        ...
        if (elapsed >= DRIP_DURATION_SECONDS) {
            ...
            newlyDripped = lastMintAmount;
        } else {
            // Linear interpolation: (amount * time_passed) / total_time
            newlyDripped = (lastMintAmount * elapsed) / DRIP_DURATION_SECONDS;
        }

        return accumulated + newlyDripped;
    }
